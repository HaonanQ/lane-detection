...
from lanedetection2 import LaneDetection
...
class Ui_MainWindow(QMainWindow):
    ...
    def run(self):
        ...
        if self.format == 1 and self.data is not None:
        ...
        elif self.format == 2 and self.data is not None:
            self.pushButton_run.setEnabled(False)
            self.timer2.start()
            self.pushButton_open.setEnabled(True)
            
    def show_input_video(self):
        if self.format == 2 and self.data.isOpened():
...
class VideoTimer(QThread):
    _signal = pyqtSignal()  # 用于触发帧更新的信号

    def __init__(self, video_fps=30):
        QThread.__init__(self)
        self.stopped = False
        self.mutex = QMutex()
        # 关键修改：接受视频实际帧率作为参数，默认30fps
        self.set_fps(video_fps)
